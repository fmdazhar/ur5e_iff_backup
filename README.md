...
3. **Clone the Repository**: Clone this repository to your local machine using the following command:
   ...
   ```bash
   git clone -b minimal https://github.com/tammerb/noetic-UR5e-Robotiq2f85.git noetic_ur
   cd noetic_ur
   git submodule init
...
- Run fake robot in MoveIt:
  ...
  ```bash
  roslaunch noetic_a_bot_moveit_config demo.launch
  ```

- Run simulated robot in Gazebo:
  ```bash
  roslaunch noetic_a_bot_moveit_config demo_gazebo.launch
  ```
...
1. **Create Your Dockerfile**: Create a new Dockerfile for your project and use `thbarkouki/noetic-ur:{specific tag}` as the base image. You can find available tags in the [Docker Hub repository](https://hub.docker.com/r/thbarkouki/noetic-ur/tags). Customize your Dockerfile to install your own project-specific dependencies.

2. **Build Your Docker Image**: Build your Docker image using your newly created Dockerfile.

3. **Customize ROS Packages**: Customize or extend the ROS packages within `catkin_ws/src/noetic_ur` to fit your project's needs. Modify existing packages or add new ones as required.
